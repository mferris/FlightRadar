...
EMPTY="plate_vs_shell plate_outside_case lip_clears_posts lip_inside_bore
       vents_clear_of_mount usbc_clears_standoffs no_turret mount_vs_plate
       antenna_clears_case mount_vs_stand"
...
for c in back_inserts_open lip_present usbc_open ant_inserts_open vents_were_under_mount turret_probe_works; do
    out="$TMP/$c.stl"
    "$SCAD" --backend=manifold -D "check=\"$c\"" -o "$out" "$DIR/checks.scad" >/dev/null 2>&1 || true
    v=$(vol_of "$out")
