...
EMPTY="plate_vs_shell plate_outside_case lip_clears_posts lip_inside_bore
       vents_clear_of_mount usbc_clears_standoffs usbc_screws_clear_window no_turret mount_vs_plate sma_mount_vs_plate connector_passes
...
for c in back_inserts_open lip_present usbc_open ant_inserts_open vents_were_under_mount socket_gauge_works cable_slot_other_side connector_gauge_works usbc_screw_probe_works turret_probe_works sma_passage_joins sma_panel_present sma_hole_open; do
    out="$TMP/$c.stl"
    "$SCAD" --backend=manifold -D "check=\"$c\"" -o "$out" "$DIR/checks.scad" >/dev/null 2>&1 || true
    v=$(vol_of "$out")
