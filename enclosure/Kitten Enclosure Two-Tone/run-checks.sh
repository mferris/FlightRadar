...
EMPTY="mount_vs_plate sma_mount_vs_plate ear_vs_post recess_vs_post whisker_through head_in_cradle
       ...
       vents_clear_of_mount usbc_clears_standoffs usbc_screws_clear_window claws_off_the_desk connector_passes
...
SEAM="material_gained paws_vs_toes body_vs_paws tail_vs_tip claws_vs_toes"
...
for c in other_screws_present back_inserts_open lip_present usbc_open ant_inserts_open vents_were_under_mount socket_gauge_works cable_slot_other_side connector_gauge_works usbc_screw_probe_works sma_passage_joins sma_panel_present sma_hole_open claws_stand_proud; do
    out="$TMP/$c.stl"
    "$SCAD" --backend=manifold -D "check=\"$c\"" -o "$out" "$DIR/checks.scad" >/dev/null 2>&1 || true
    v=$(vol_of "$out")
