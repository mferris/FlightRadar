...
EMPTY="mount_vs_plate ear_vs_post recess_vs_post whisker_through head_in_cradle
       ...
       socket_takes_base connector_has_room"
...
for c in other_screws_present back_inserts_open lip_present usbc_open ant_inserts_open vents_were_under_mount socket_gauge_works connector_gauge_works claws_stand_proud; do
    out="$TMP/$c.stl"
    "$SCAD" --backend=manifold -D "check=\"$c\"" -o "$out" "$DIR/checks.scad" >/dev/null 2>&1 || true
    v=$(vol_of "$out")
